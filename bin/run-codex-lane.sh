...
TMP_DIR=$(mktemp -d -t run-codex-lane.XXXXXX) || exit 70
cleanup() {
...
FINAL_FILE=$(mktemp "$TMP_DIR/final.XXXXXX") || exit 70
CLI_OUTPUT_FILE=$(mktemp "$TMP_DIR/cli-output.XXXXXX") || exit 70
...
if [[ -z $CODEX_BIN ]]; then
  : > "$FINAL_FILE"
  printf 'ERROR: codex was not found at /opt/homebrew/bin/codex or on PATH\n' > "$CLI_OUTPUT_FILE"
  CLI_EXIT=127
else
  CODEX_COMMAND=(
    "$CODEX_BIN" exec
  ...
  if [[ -n $TIMEOUT_BIN ]]; then
    "$TIMEOUT_BIN" 600 "${CODEX_COMMAND[@]}" < "$SPEC" > "$CLI_OUTPUT_FILE" 2>&1
  else
    printf 'WARN: no timeout binary is available; codex runs uncapped\n' >&2
    "${CODEX_COMMAND[@]}" < "$SPEC" > "$CLI_OUTPUT_FILE" 2>&1
  fi
  CLI_EXIT=$?
fi
